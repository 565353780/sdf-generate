...
def process_mesh(
    ...
    if sharp_edges_count > 0:
        sharp_edge_link_normal = np.array(
            np.concatenate([link_normal1, link_normal2], axis=1)
        )
    ...
    if sharp_edges_count > 0:
        ...
        sharp_vertices_pair = mesh.vertices[
            sharp_edges_vertices_pair
        ]  # 顶点对坐标 1225，2，3
        # sharp_edge_link_normal = data['sharp_edge_link_normal']
        epsilon = 1e-4  # 一个小的数值
        edge_normal = (
            0.5 * sharp_edge_link_normal[:, :3] + 0.5 * sharp_edge_link_normal[:, 3:]
        )
        ...
        if (
            num_known_vertices < num_target_sharp_vertices
        ):  # 如果已知的顶点数量小于目标的顶点数量
            num_new_vertices = num_target_sharp_vertices - num_known_vertices
            if (
                num_new_vertices >= sharp_edge_length
            ):  # 如果需要补充的顶点数量大于sharp edge的个数,说明每一条sharp edge之间至少需要插值一个新的顶点
                num_new_vertices_per_pair = (
                    num_new_vertices // sharp_edge_length
                )  # 计算每个顶点对对（边）均匀分配的顶点数
                new_vertices = np.zeros(
                    (sharp_edge_length, num_new_vertices_per_pair, 6)
                )  # 初始化新顶点数组 在每一个shapr edge中插值
                ...
                if remaining_vertices > 0:
                    ...
                    ind = rng.choice(
                        sharp_edge_length, remaining_vertices, replace=False
                    )
                    new_vertices_remain = np.zeros(
                    ...
                    edge_normal = (
                        0.5 * sharp_edge_link_normal[ind, :3]
                        + 0.5 * sharp_edge_link_normal[ind, 3:]
                    )
                    edge_normal = edge_normal / np.linalg.norm(
                        edge_normal, axis=1, keepdims=True
                    )
                    new_vertices_remain[:, 3:] = edge_normal

                    new_vertices = np.concatenate(
                        [new_vertices, new_vertices_remain], axis=0
                    )
            else:
                remaining_vertices = (
                    num_new_vertices % sharp_edge_length
        ...
        sharp_near_surface_points = [
            sharp_surface_points
            + np.random.normal(scale=0.001, size=(len(sharp_surface_points), 3)),
            sharp_surface_points
            + np.random.normal(scale=0.005, size=(len(sharp_surface_points), 3)),
            sharp_surface_points
            + np.random.normal(scale=0.007, size=(len(sharp_surface_points), 3)),
            sharp_surface_points
            + np.random.normal(scale=0.01, size=(len(sharp_surface_points), 3)),
        ]
        ...
        sharp_near_surface = np.concatenate(
            [sharp_near_surface_points, sharp_sdf], axis=1
        )  # sharp 近表面点的位置及其sdf
        ...
        coarse_near_surface_points = [
            coarse_surface_points
            + np.random.normal(scale=0.001, size=(len(coarse_surface_points), 3)),
            coarse_surface_points
            + np.random.normal(scale=0.005, size=(len(coarse_surface_points), 3)),
        ]
        ...
        rand_points = np.concatenate(
            [rand_points, coarse_sdf], axis=1
        )  # rand points包括coarse 近表面的和空间中均匀分布的点的位置及其sdf
        ...
        for _ in range(1):
            ...
            fps_coarse_surface = coarse_surface[kdline_fps_samples_idx].reshape(
                -1, 1, 6
            )
        ...
        if sharp_surface.shape[0] > num_target_sharp_vertices:
            for _ in range(1):
                ...
                fps_sharp_surface = sharp_surface[kdline_fps_samples_idx].reshape(
                    -1, 1, 6
                )
